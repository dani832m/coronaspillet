...
const updateGameArea = () => {
  ...
  for (i = 0; i < obstacles.length; i += 1) {
    ...
    if (gamePiece.crashWith(obstacles[i])) {
      ...
      restart = new Component("18px", "Arial", "white", 98, 160, "text");
      died.text = "Du smittede! Spillet er slut.";
      restart.text = "Tryk på 'Restart' for at prøve igen.";
      died.update();
      restart.update();
      bgMusic.stop();
...
document.querySelector(".bottom-panel-restart-btn").addEventListener("touchstart", () => {
  bgMusic.stop();
  gameArea.stop();
  gameArea.clear();
  obstacles = [];
